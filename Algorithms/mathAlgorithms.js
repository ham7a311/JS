...
Recursive Case: 
        For n > 0, return n multiplied by the factorial of n−1. This continues until the base case is reached.
...
Loop:  A for loop iterates from index 2 to n−1.
...
Base Case: 
        If n is less than 2 (i.e., n=0 or n=1), the function returns n. This handles the first two Fibonacci numbers directly.
...
Base Case: 
        The function first checks if n is less than 2. Since prime numbers are defined as greater than 1, it returns false for any number less than 2.

Loop Through Possible Divisors: 
        The function then enters a for loop that iterates from 2 to the square root of nnn (inclusive). This is based on the mathematical principle that if n is divisible by any number greater than its square root, it must also be divisible by a number smaller than its square root.

Divisibility Check: 
        Within the loop, the function checks if nnn is divisible by iii (i.e., if n % i === 0). If it finds any divisor, it returns false, indicating that n is not a prime number.
...
Base Case: 
        The function first checks if nnn is less than 1. Since powers of two are defined for positive integers only (i.e., 1,2,4,8,…1, 2, 4, 8, \ldots1,2,4,8,…), it returns false for any non-positive integer.

Iterative Division: 
        The function enters a while loop that continues as long as nnn is greater than 1. Within the loop:
...
Return True: 
        If the loop completes and n is reduced to 1, the function returns true, indicating that the original number was indeed a power of two.
...
Base Case: 
        The function first checks if n is less than 1. Since powers of two are defined for positive integers only (i.e., 1,2,4,8,…), it returns false for any non-positive integer.

Bitwise Operation: 
        The core of the function is the expression (n & (n - 1)) === 0. This expression utilizes a property of binary numbers:
...
Return Value: 
        If the result of the bitwise AND operation is zero, the function returns true, indicating that n is a power of two. Otherwise, it returns false.
